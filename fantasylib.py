...
def populatelocaljson():  # load live json from mls website
    async def get(url, session):
        try:
            async with session.get(url=url[0]) as response:
                ...
                with open('jsonlcl/' + str(url[1]) + '.json', 'w') as outfile:
    ...
    players = requests.get(
        url='https://fgp-data-us.s3.us-east-1.amazonaws.com/json/mls_mls/players.json?_=1646966560478').json()
    #teams = requests.get(
    #    url='https://fgp-data-us.s3.us-east-1.amazonaws.com/json/mls_mls/squads.json?_=1646966560478').json()
    rounds = requests.get(
        url='https://fgp-data-us.s3.us-east-1.amazonaws.com/json/mls_mls/rounds.json?_=1646966560474').json()
    ...
    for i in range(len(players)):
        ...
        websites.append([url, player["id"]])
    asyncio.run(main(websites))


def getlocaljson(directory):
    with open(directory + "/players.json", "r") as infile:
        players = json.load(infile)
    with open(directory + "/squads.json", "r") as infile:
        teams = json.load(infile)
    with open(directory + "/rounds.json", "r") as infile:
        rounds = json.load(infile)
    return players, teams, rounds


def setup(directory):
    players, teams, rounds = getlocaljson(directory)
    ...
    for Round in rounds:
        ...
        for game in Gameweek:
            ...
            gw.append(g)
        Gameweeks.append(gw)
    return players, teams, rounds, playerlist, teamlist, Gameweeks


def teamselect(CG, players, RMS, objFun):
    ...
    def create_data_model():
        ...
        data['num_constraints'] = 34  # team constraints: 29 , position constraints: 4, cost constraints: 1 = 33
        return data
    ...
    for val in positions:
        for player in squad:
            if player.position == val:
                print(player.name + ',' + str(player.position) + ',' + player.team.long + ',' + str(player.cost)+ ',' + str(player.pointhistory[0]))
                cost = cost + player.cost
                points = points + player.pointhistory[0]
